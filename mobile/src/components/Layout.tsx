...
const Layout: React.FC<LayoutProps> = ({ children, currentScreen, setScreen, onSelectEmployee, onSelectAssignment }) => {
    ...
    const menuItems = [
        ...
        { id: 'MyTasks', name: 'My Tasks', icon: 'check-square', roles: ['Employee', 'Vendor'] },
        { id: 'Employees', name: 'Employees', icon: 'users', roles: ['Admin', 'HR'] },
        { id: 'Vendors', name: 'Vendors', icon: 'briefcase', roles: ['Admin', 'HR'] },
        { id: 'VendorBills', name: 'Vendor Bills', icon: 'file-text', roles: ['Admin', 'HR', 'Vendor'] },
        { id: 'OfferLetters', name: 'Offers', icon: 'mail', roles: ['Admin', 'HR'] },
        { id: 'Workflows', name: 'Workflows', icon: 'git-merge', roles: ['Admin', 'HR'] },
        { id: 'Alerts', name: 'Alerts', icon: 'bell', roles: ['Admin', 'HR'] },
        { id: 'Reports', name: 'Reports', icon: 'bar-chart-2', roles: ['Admin', 'HR', 'Employee', 'Vendor'] },
        { id: 'Documents', name: 'Documents', icon: 'folder', roles: ['Admin', 'HR'] },
    ];

    const filteredMenu = menuItems.filter(item => item.roles.includes(user?.role || ''));

    const detailScreens = ['EmployeeProfile', 'AssignmentDetail', 'AssignTask', 'Profile'];
    const isDetail = detailScreens.includes(currentScreen);
        ...
        <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
            ...
            <View style={styles.header}>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                    ...
                    <TouchableOpacity onPress={() => { loadNotifications(); setShowNotif(true); }} style={styles.iconBtn}>
                        <Feather name="bell" size={18} color="#64748b" />
                        {unreadCount > 0 && <View style={styles.badgeDot} />}
                    </TouchableOpacity>
                    <TouchableOpacity onPress={() => setScreen('Profile')} style={styles.profileBtn}>
                        <Text style={styles.profileTxt}>
                            {(user?.role === 'Vendor' ? (user?.companyName || user?.name) : user?.name || 'U').charAt(0).toUpperCase()}
                        </Text>
                    </TouchableOpacity>
                </View>
            </View>
                            ...
                            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                                {notifications.length > 0 && (
                                    <TouchableOpacity onPress={markAllRead}>
                                        <Text style={{ fontSize: 10, fontWeight: '800', color: '#2563eb' }}>MARK ALL</Text>
                                    </TouchableOpacity>
                                )}
                                ...
                                renderItem={({ item }) => {
                                    ...
                                    return (
                                        <TouchableOpacity
                                            ...
                                            style={{
                                                flexDirection: 'row', marginBottom: 12, borderBottomWidth: 1,
                                                borderBottomColor: '#f1f5f9', paddingBottom: 12,
                                                opacity: isUnread ? 1 : 0.65
                                            }}
                                        >
                                            <View style={{
                                                width: 32, height: 32, backgroundColor: isUnread ? '#eff6ff' : '#f8fafc',
                                                borderRadius: 8, alignItems: 'center', justifyContent: 'center', marginRight: 12
                                            }}>
                                                <Feather name="bell" size={14} color={isUnread ? '#3b82f6' : '#94a3b8'} />
                                            </View>
                                            <View style={{ flex: 1 }}>
                            ...
                            {!searchQuery && (
                                <View style={{ padding: 32, alignItems: 'center' }}>
                                    ...
                                    <Text style={{ fontSize: 11, fontWeight: '500', color: '#64748b', textAlign: 'center', marginTop: 6, lineHeight: 16 }}>
                                        Query employee names, onboarding workflows, offer letters, or status.
                                    </Text>
                                </View>
                            )}
                                        ...
                                        >
                                            <View style={styles.searchResultLeft}>
                                                <View style={[styles.searchResultIcon, { backgroundColor: '#eff6ff' }]}>
                                                    <Feather name={item.role === 'Vendor' ? 'briefcase' : 'user'} size={14} color="#3b82f6" />
                                                </View>
                                                <View>
                                                    <Text style={styles.searchResultTitle}>{item.companyName || item.name}</Text>
                                                    <Text style={styles.searchResultSubtitle}>{item.role} • {item.department || item.vendorType || 'No Department'}</Text>
                                                </View>
                                            </View>
...
const styles = StyleSheet.create({
    ...
    logoImage: { width: 32, height: 32, borderRadius: 8, marginRight: 12 },
    brand: { fontSize: 16, fontWeight: '800', color: '#0f172a', letterSpacing: -0.5 },
    brandSub: { fontSize: 8, fontWeight: '900', color: '#3b82f6', letterSpacing: 1.5, marginTop: 2 },
    iconBtn: {
        width: 36, height: 36, alignItems: 'center', justifyContent: 'center',
        backgroundColor: '#f8fafc', borderRadius: 18, marginRight: 12, position: 'relative'
    },
    badgeDot: {
        width: 8, height: 8, backgroundColor: '#ef4444', borderRadius: 4,
        position: 'absolute', top: 8, right: 8, borderWidth: 1, borderColor: 'white'
    },
    profileBtn: { width: 36, height: 36, backgroundColor: '#0f172a', borderRadius: 18, alignItems: 'center', justifyContent: 'center' },
    profileTxt: { color: 'white', fontSize: 12, fontWeight: '800' },
    content: { flex: 1, backgroundColor: '#f8fafc' },
    tabContainer: { backgroundColor: 'white', borderTopWidth: 1, borderTopColor: '#e2e8f0', paddingBottom: Platform.OS === 'ios' ? 20 : 0 },
    scrollContent: { paddingVertical: 12, paddingHorizontal: 16 },
    tabItem: {
        flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 8,
        borderRadius: 4, marginRight: 8, backgroundColor: 'transparent'
    },
